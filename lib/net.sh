...
hl_dns_hijacked() {
    ...
    while read -r ns; do
        ...
        # ...и третий октет принадлежит РЕАЛЬНОМУ модему.
        #
        # Именно порту, а не просто «занятому номеру»: в HL_MAP_USED лежат
        # ещё и подсети, зарезервированные за самим хостом. Из-за этого
        # штатный LAN-шлюз 192.168.1.1 опознавался как модем слота 1.
        local p
        for p in "${!HL_MAP_PORT2SLOT[@]}"; do
            if [ "${HL_MAP_PORT2SLOT[$p]}" = "$n" ]; then
                dbg "резолвер указывает на модем слота $n ($ns), порт $p"
                return 0
            fi
        done
