...
if [ "${SKIP_RNDIS_FIX:-0}" = 1 ]; then
    ...
    if bash "$PREFIX_LIB/dkms/build.sh" 2>&1 | sed 's/^/    /'; then
        ...
        warn "    сборка фикса не прошла — парк заработает, но приём будет ~1.3 Мбит"
        warn "    разбор: bash $PREFIX_LIB/dkms/build.sh"
    fi
    # Модуль подменён на диске; перезагружаем его сейчас, до подъёма
    # интерфейсов, иначе фикс заработает только после ребута.
